...
class MirrorMyManga:
    ...
    def transform_pdf(self, input_path: str, output_path: str, dpi=200, show_logs=False, verbose=False):
        ...
        if not os.path.exists(os.path.join(Path.cwd(), file_name)):
            if verbose:
                print("LOG: input dir doesn't exist creating new one")
            os.mkdir(os.path.join(Path.cwd(), file_name))
        ...
        with fitz.open(input_path) as doc:
            ...
            for i in range(len(doc)):
                for img in doc.get_page_images(i):
                    ...
                    image.save(os.path.join(os.path.join(Path.cwd(), file_name), f"{i}.jpeg"))
                    i += 1

        for i, fname in enumerate(os.listdir(os.path.join(Path.cwd(), file_name))):
            if fname.endswith(".png") or fname.endswith(".jpg") or fname.endswith(".jpeg"):
                start = time.perf_counter()
                panel = cv2.imread(f"/{os.path.join(Path.cwd(), file_name)}/{fname}")
                panel = self.transform(panel, show_logs=show_logs, show_panel=False)
        ...
        shutil.rmtree(os.path.join(Path.cwd(), file_name))
...
if __name__ == "__main__":
    ...
    input_path = "/home/vhvhs/Downloads/[Meganebuk] Vagabond Volume 1 (Eng).pdf"
    output_path = os.path.join(Path.cwd(), "Exp [Meganebuk] Vagabond Volume 1 (Eng) Result.pdf")
